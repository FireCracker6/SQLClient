...
namespace SQLClient_Dapper.Services
{
    ...
    {
        private readonly string _connectionString = @"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\Users\Svart\Source\Repos\Webbutveckling\SqlClient\SQLClient_Dapper\Data\localSQL_dapper_db.mdf;Integrated Security=True;Connect Timeout=30";
        ...
        {
            using var conn = new SqlConnection(_connectionString);
            return await conn.QueryAsync<Customer>("SELECT c.Id, c.FirstName, c.LastName, c.Email, c.PhoneNumber, a.StreetName, a.PostalCode, a.City FROM Customers c JOIN Addresses a ON c.AddressId = a.Id");
        }

        public async Task<Customer> GetCustomerAsync(string email)
        {
            using var conn = new SqlConnection(_connectionString);
            return await conn.QueryFirstOrDefaultAsync<Customer>("SELECT c.Id, c.FirstName, c.LastName, c.Email, c.PhoneNumber, a.StreetName, a.PostalCode, a.City FROM Customers c JOIN Addresses a ON c.AddressId = a.Id WHERE c.Email = @Email", new {Email = email});
        }
